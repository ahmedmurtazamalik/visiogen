...
def build_claim_prompt() -> str:
    ...
    return (
        ...
        "relationships or properties. Alias, role, containment, connection, direction, "
        "relationship type, branch condition, cardinality, reference mapping, sequence, and "
        "attribute/state claims require object_text. A sequence must be one atomic subject-to-"
        "object step, never a multi-entity chain stored only in subject_text. Normalize "
...
def build_claim_repair_prompt(
    ...
    return (
        ...
        "scope errors in the previous claim response. Do not introduce proposition content or "
        "source text absent from the previous response; a non-atomic composite claim may be "
        "split into atomic claims supported by the same cited source. Return the complete "
        "corrected batch.\n\n"
